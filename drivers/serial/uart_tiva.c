...
/**
 * @brief Driver for TI Tiva UART
 *
 * TI Tiva UART Driver.   Derived from LM3S6965 Driver.
 * 
...
{
	const struct uart_tiva_config *config = dev->config;
...
{
	struct uart_tiva_dev_data_t *data = dev->data;
	const struct uart_tiva_config *config = dev->config;
	disable(dev);
 ...
 */
static void uart_tiva_poll_out(const struct device *dev,
					     unsigned char c)
{
	const struct uart_tiva_config *config = dev->config;
 ...
 */
static int uart_tiva_fifo_fill(const struct device *dev,
 ...
 */
static int uart_tiva_fifo_read(const struct device *dev,
				    uint8_t *rx_data,
				    const int size)
{
	const struct uart_tiva_config *config = dev->config;
 ...
 */
static void uart_tiva_irq_callback_set(const struct device *dev,
					    uart_irq_callback_user_data_t cb,
					    void *cb_data)
{
	struct uart_tiva_dev_data_t * const dev_data = dev->data;
...
static const struct uart_tiva_config uart_tiva_dev_cfg_0 = {
...
static struct uart_tiva_dev_data_t uart_tiva_dev_data_0 = {
...
DEVICE_DT_INST_DEFINE(0,
		    &uart_tiva_init,
		    NULL,
		    &uart_tiva_dev_data_0, &uart_tiva_dev_cfg_0,
...
static void irq_config_func_0(const struct device *dev)
{
	IRQ_CONNECT(DT_INST_IRQN(0),
		    ...
		    uart_tiva_isr, DEVICE_DT_INST_GET(0),
		    0);
	irq_enable(DT_INST_IRQN(0));
}
#endif

#endif /* CONFIG_UART_TIVA_PORT_0 */
...
static struct uart_tiva_config uart_tiva_dev_cfg_1 = {
...
static struct uart_tiva_dev_data_t uart_tiva_dev_data_1 = {
...
DEVICE_DT_INST_DEFINE(1,
		    &uart_tiva_init,
		    NULL,
		    &uart_tiva_dev_data_1, &uart_tiva_dev_cfg_1,
		    PRE_KERNEL_1, CONFIG_SERIAL_INIT_PRIORITY,
		    &uart_tiva_driver_api);
...
static void irq_config_func_1(const struct device *dev)
{
	IRQ_CONNECT(DT_INST_IRQN(1),
		    ...
		    uart_tiva_isr, DEVICE_DT_INST_GET(1),
		    0);
	irq_enable(DT_INST_IRQN(1));
}
#endif

#endif /* CONFIG_UART_TIVA_PORT_1 */
...
static const struct uart_tiva_config uart_tiva_dev_cfg_2 = {
...
static struct uart_tiva_dev_data_t uart_tiva_dev_data_2 = {
...
DEVICE_DT_INST_DEFINE(2,
		    &uart_tiva_init,
		    NULL,
		    &uart_tiva_dev_data_2, &uart_tiva_dev_cfg_2,
		    PRE_KERNEL_1, CONFIG_SERIAL_INIT_PRIORITY,
		    &uart_tiva_driver_api);
...
static void irq_config_func_2(const struct device *dev)
{
	IRQ_CONNECT(DT_INST_IRQN(2),
		    ...
		    uart_tiva_isr, DEVICE_DT_INST_GET(2),
		    0);
	irq_enable(DT_INST_IRQN(2));
}
#endif

#endif /* CONFIG_UART_TIVA_PORT_2 */
